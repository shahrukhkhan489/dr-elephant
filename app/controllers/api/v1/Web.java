...
public class Web extends Controller {
 ...
 /**
  * Returns the list of AppResults for the given name limit by maxApplications
  * @param name The name for which applications need to be fetched.
 ...
 /**
  * Returns the list of AppResults scheduled by a scheduler for the given name limit by maxApplications.
  * @param name The name for which applications need to be fetched.
  * @param maxApplications The max number of applications that should be fetched
 ...
 private static List<AppResult> getSchedulerApplications(String name, int maxApplications, int nullvalue) {
   List<AppResult> results =
       AppResult.find.select("*").where().like(AppResult.TABLE.NAME, "%" + name + "%").ne(AppResult.TABLE.FLOW_EXEC_ID, null)
...
// ################################################################# START ########################################################################
 ...
 /**
  * This method returns the json object for the application-summaries based on the name
  * @param name The name for which application-summaries json must be returned
  * @return The application-summaries json for the given name
  * response object:
  * <pre>
  *{
  ...
  * </pre>
  * */
 public static Result restApplicationSummariesForName(String name) {
   ...
   for (AppResult application : results) {
     ...
     applicationObject.addProperty(JsonKeys.NAME, application.name);
 ...
 /**
  * This method returns the json object for job-summaries for the given name
  * @param name The given name for which job-summaries json object should be returned
  * @return The job-summaries json object for the given name
  * response object:
  * <pre>
  *{
  *  "job-summaries": [
  ...
  * @param name The name for which workflow-summaries must be returned
  * @return The json response of the workflow-summaries for the given name
  * Response data:
  * <pre>
  *{
  *  "workflow-summaries": [
  ...
  * </pre>
  */
 public static Result restWorkflowSummariesForName(String name) {
   ...
   for (IdUrlPair flowExecPair : keyList) {
     ...
     dataset.addProperty(JsonKeys.NAME, mrJobsList.get(0).name);
 ...
 /**
  *  The rest interface to return the results for a particular name. When the date is not specified, it returns the result
  ...
  *
  */
 public static Result restGetNamesSummaryStats() {
   ...
   String nameString = form.get("names");
   if (nameString == null || nameString.isEmpty()) {
     JsonObject parent = new JsonObject();
     parent.add(JsonKeys.NAME_RESULTS, new JsonObject());
     return notFound(new Gson().toJson(parent));
   }

   List<String> names = Arrays.asList(nameString.split(","));

   Map<String, String> filterParamsForNameSummary = getFilterParamsForNameSummary();
   ...
   for (int i = 0; i < names.size(); i++) {
     nameQueryList.add("name=:name" + i);
   }

   String nameQueryString = StringUtils.join(nameQueryList, " or ");
   ...
   if (Utils.isSet(filterParamsForNameSummary.get(Application.FINISHED_TIME_BEGIN))) {
     finishedTimeBegin = filterParamsForNameSummary.get(Application.FINISHED_TIME_BEGIN);
   }

   if (Utils.isSet(filterParamsForNameSummary.get(Application.FINISHED_TIME_END))) {
     finishedTimeEnd = filterParamsForNameSummary.get(Application.FINISHED_TIME_END);
   }
   ...
   if (timeFilterString != null && !timeFilterString.isEmpty()) {
     sqlBuilder.append(" ( ");
     sqlBuilder.append(nameQueryString);
   ...
   for (String name : names) {
     query.setParameter("name" + iNameIndex, name);
     iNameIndex++;
   }

   SqlRow resultRow = query.findUnique();
   nameResult.addProperty(JsonKeys.ID, nameString);
   nameResult.addProperty(JsonKeys.TOTAL_APPLICATIONS, resultRow.getLong("num_of_applications"));
   nameResult.addProperty(JsonKeys.TOTAL_JOBS, resultRow.getLong("num_of_jobs"));
   nameResult.addProperty(JsonKeys.TOTAL_WORKFLOWS, resultRow.getLong("num_of_flows"));
   nameResult.addProperty(JsonKeys.RESOURCE_USED, resultRow.getLong("total_resource_used"));
   nameResult.addProperty(JsonKeys.RESOURCE_WASTED, resultRow.getLong("total_resource_wasted"));
   nameResult.addProperty(JsonKeys.RUNTIME, resultRow.getLong("execution_time"));
   nameResult.addProperty(JsonKeys.WAITTIME, resultRow.getLong("total_delay"));

   Query<AppResult> nameSummaryQuery =
       generateNameApplicationSummaryQuery(names, filterParamsForNameSummary, sortBy, increasing);

   total = nameSummaryQuery.findRowCount();

   List<AppResult> results = nameSummaryQuery.setFirstRow(offset).setMaxRows(limit)
       .fetch(AppResult.TABLE.APP_HEURISTIC_RESULTS, AppHeuristicResult.getSearchFields()).findList();
   ...
   nameResult.addProperty(JsonKeys.START, offset);
   nameResult.addProperty(JsonKeys.END, end);
   nameResult.addProperty(JsonKeys.TOTAL, total);
   nameResult.add(JsonKeys.SUMMARIES, applicationSummaryArray);

   JsonObject parent = new JsonObject();
   parent.add(JsonKeys.NAME_DETAILS, nameResult);
   return ok(new Gson().toJson(parent));
 }
...
public static Result restGetUsersSummaryStats() {
  ...
  * @return The Query object based on the given above parameters
  */
 public static Query<AppResult> generateNameApplicationSummaryQuery(List<String> names,
     Map<String, String> searchParams, String sortKey, boolean increasing) {
   ExpressionList<AppResult> query = AppResult.find.select(AppResult.getSearchFields()).where();
   Junction<AppResult> junction = query.disjunction();
   for (String name : names) {
     junction.eq(AppResult.TABLE.NAME, name);
   }
   query.endJunction();
